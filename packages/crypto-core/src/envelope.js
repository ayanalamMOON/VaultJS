...
const { AES_ALGO, AES_IV_BYTES, HMAC_ALGO, TOKEN_VERSION, TOKEN_CRYPTO_SUITE } = require('./constants');
...
function decryptPayload(encrypted, aesKey, aad = '') {
    if (encrypted.suite && encrypted.suite !== TOKEN_CRYPTO_SUITE) {
        throw new Error('unsupported token suite');
    }
    if (encrypted.alg && encrypted.alg !== AES_ALGO) {
...
function verifyAndParseEnvelope(token, hmacKey) {
    ...
    if (parsed.suite && parsed.suite !== TOKEN_CRYPTO_SUITE) {
        throw new Error('unsupported token suite');
    }
    if (parsed.alg && parsed.alg !== AES_ALGO) {
        throw new Error('unsupported token algorithm');
    }
    return parsed;
}
